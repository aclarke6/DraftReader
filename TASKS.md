...
- [OPEN] `diff-para--removed` renders struck-through deleted text in reader views — should render as an empty thin red left-bar marker only, not display removed HTML content. Reader focus must be on what is present, not what was deleted. Fix: render empty `<div class="diff-para diff-para--removed"></div>` and update `.diff-para--removed` CSS to show only a left border with no content. Files: `DesktopRead.cshtml`, `MobileRead.cshtml`, `DraftView.DesktopReader.css`. Defer until after V-Sprint 3 is merged to main.
