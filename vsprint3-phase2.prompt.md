...
Populate this as a return value from `ResolveSceneContentAndDiffAsync`, or add
`UpdatedSinceLastRead` to the tuple returned by that method.

Read the current return tuple signature before modifying. Extend it to include
`bool updatedSinceLastRead`:
...
Find the scene content area — the block that renders `item.Scene.Title` and
the prose content. Add the inline message between the title and the prose:
...
Position this immediately before the prose render block (before the
`scene-prose-content` div), inside the scene loop.
...
Add the notice class. If the class is also needed in mobile, add it to
`DraftView.MobileReader.css` or to `DraftView.Core.css` if shared:
