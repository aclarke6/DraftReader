...
namespace DraftReader.Infrastructure.Tests.Persistence;

public class ScrivenerProjectRepositoryTests : IDisposable
{
    private readonly DraftReaderDbContext _db;
    private readonly ScrivenerProjectRepository _sut;

    public ScrivenerProjectRepositoryTests()
    {
        var options = new DbContextOptionsBuilder<DraftReaderDbContext>()
            .UseInMemoryDatabase(Guid.NewGuid().ToString())
            .Options;

        _db  = new DraftReaderDbContext(options);
