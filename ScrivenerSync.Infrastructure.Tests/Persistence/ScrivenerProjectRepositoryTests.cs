...
namespace ScrivenerSync.Infrastructure.Tests.Persistence;

public class ScrivenerProjectRepositoryTests : IDisposable
{
    private readonly ScrivenerSyncDbContext _db;
    private readonly ScrivenerProjectRepository _sut;

    public ScrivenerProjectRepositoryTests()
    {
        var options = new DbContextOptionsBuilder<ScrivenerSyncDbContext>()
            .UseInMemoryDatabase(Guid.NewGuid().ToString())
            .Options;

        _db  = new ScrivenerSyncDbContext(options);
