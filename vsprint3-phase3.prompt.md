...
Extend `ResolveSceneContentAndDiffAsync` to return `showUpdateBanner` and
`currentVersionNumber` (if not already returned from Phase 2).
...
(choose the position that makes structural sense given the existing layout):
...
In both `DesktopRead.cshtml` and `MobileRead.cshtml`, add a script block (or extend
the existing scripts section) to handle the dismiss button click:
...
The POST is fire-and-forget — no need to await or handle the response in JS.
The banner is removed from the DOM immediately on click regardless of server response.

Ensure an antiforgery token input is available on the page. Check if
`@Html.AntiForgeryToken()` is already present in the view — if not, add it.
...
If the `.scene-updated-notice` from Phase 2 and the `.update-banner` share colour
variables, ensure both use the same token. Do not introduce duplicate CSS variables.

Add the same styles to `DraftView.MobileReader.css` if the mobile view requires
different sizing — otherwise the classes in `DraftView.DesktopReader.css` are
sufficient if that stylesheet is loaded on mobile views.

Check `_Layout.cshtml` to confirm both stylesheets are loaded. If the desktop reader
CSS is not loaded on mobile views, move the shared banner styles to `DraftView.Core.css`.
